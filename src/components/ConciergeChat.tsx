...
export default function ConciergeChat() {
  ...
  const send = () => {
    if (!msg.trim()) return;
    setLines((l) => [...l, `You: ${msg}`, "Concierge: Noted. We’ll prepare a phased blueprint."]);
    setMsg("");
  };
      ...
      {open && (
        <div className="w-[300px] p-4 rounded-xl glass-glow bg-ink/70">
          ...
          <div className="mt-3 flex gap-2">
            ...
            />
            <button className="px-3 py-1 rounded-md bg-white text-ink text-xs" onClick={send}>Send</button>
          </div>
        </div>
      )}
      <button id="conciergeButton" className="px-4 py-2 rounded-full bg-white text-ink text-sm shadow-lg" onClick={() => setOpen((v) => !v)}>
