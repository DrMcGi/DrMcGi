...
import BlueprintGenerator from "@/components/BlueprintGenerator"; // ✅ typed default import
import { features } from "@/utils/features";

export default function HomePage() {
  const [selectedFeatures, setSelectedFeatures] = useState<Set<string>>(new Set(["responsive", "seoOptimization", "luxuryUI"]));

  const computeTotal = () => {
    let base = 0;
    features.forEach((f) => { if (selectedFeatures.has(f.id)) base += f.price; });
  ...
  return (
    <>
      ...
      <main className="pt-20">
        <HeroSection />
        <TailoredPackages />
        <PackageSelector onSelect={(feat) => setSelectedFeatures(new Set(feat))} />
        <BuildConfigurator selected={selectedFeatures} onChange={(next) => setSelectedFeatures(next)} />
        <PricingTiers onSelect={(modules) => setSelectedFeatures(new Set(modules))} />
